/*
 * gh_zero_engine — backend contract
...
pub(crate) trait Backend: Sized {
    ...
    // Complete an already-recorded graph and return its greedy token. Device
    // backends may override this to append reduction/readback before the single
    // submission; the portable path preserves the existing two-step lifecycle.
    fn submit_argmax(
        &self,
        enc: Self::Encoder,
        logits: &Self::Buffer,
...
mod tests {
    ...
    impl ProbeBackend {
        fn new(fail_submit: bool, fail_read: bool) -> Self {
            Self {
                submits: Cell::new(0),
                reads: Cell::new(0),
                fail_submit,
    ...
    impl Backend for ProbeBackend {
        ...
        fn load(
            _: &crate::gguf::metadata::ModelMetadata,
            _: &dyn crate::backend::source::WeightSource,
            _: &crate::gguf::loader::GgufFile,
            _: usize,
        ) -> Result<Self> {
            unreachable!("contract probe is constructed directly")
        }

        fn buffers(&self) -> &crate::backend::buffers::Buffers<()> {
            unreachable!("contract probe owns no graph buffers")
        }

        fn alloc_buffer(&self, _: u64) -> Result<()> {
            Ok(())
        }

        fn free_buffer(&self, _: ()) {}

        fn view(&self, _: &(), _: u64, _: u64) {}
        ...
        fn embed(&self, _: &(), _: &(), _: &(), _: u32, _: u32) -> Result<()> {
            Ok(())
        }

        fn matmul(&self, _: &(), _: &(), _: &(), _: &(), _: u32, _: u32) {}

        fn logits(&self, _: &(), _: &(), _: &(), _: &(), _: u32, _: u32) {}

        fn rmsnorm_x(&self, _: &(), _: &(), _: &(), _: &(), _: u32, _: f32, _: u32) {}
        ...
        fn rope_yarn(
            ...
            _: u32,
            _: u32,
            _: &crate::backend::rope::Yarn,
            _: crate::backend::rope::RopeRole,
        ) -> Result<()> {
            Ok(())
        }

        fn silu_mul(&self, _: &(), _: &(), _: &(), _: &(), _: u32) {}

        fn residual_add(&self, _: &(), _: &(), _: &(), _: u32) {}
        ...
        fn read_logits(&self, _: &(), vocab: usize) -> Result<Vec<f32>> {
            Ok(vec![0.0; vocab])
        }

        fn read_argmax(&self, _: &(), vocab: usize) -> Result<u32> {
            self.reads.set(self.reads.get() + 1);
            if self.fail_read || vocab == 0 {
                bail!("injected argmax read failure");
            }
            // The probe models equal maxima at indices 0 and 2: lowest wins.
            Ok(0)
        }

        fn read_topk(&self, _: &(), _: usize, _: usize) -> Result<Vec<(u32, f32)>> {
            Ok(Vec::new())
        }
    }

    #[test]
    fn default_submit_argmax_submits_once_then_reads_lowest_tie() {
        let backend = ProbeBackend::new(false, false);
        assert_eq!(backend.submit_argmax((), &(), 3).unwrap(), 0);
        assert_eq!((backend.submits.get(), backend.reads.get()), (1, 1));
    }

    #[test]
    fn default_submit_argmax_does_not_read_after_submit_failure() {
        let backend = ProbeBackend::new(true, false);
        assert!(backend.submit_argmax((), &(), 3).is_err());
        assert_eq!((backend.submits.get(), backend.reads.get()), (1, 0));
    }

    #[test]
    fn default_submit_argmax_returns_read_failure_after_submit() {
        let backend = ProbeBackend::new(false, true);
        assert!(backend.submit_argmax((), &(), 3).is_err());
        assert_eq!((backend.submits.get(), backend.reads.get()), (1, 1));
    }

    #[test]
    fn default_submit_argmax_preserves_empty_vocab_error() {
        let backend = ProbeBackend::new(false, false);
        assert!(backend.submit_argmax((), &(), 0).is_err());
        assert_eq!((backend.submits.get(), backend.reads.get()), (1, 1));
    }
}
