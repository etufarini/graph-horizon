/*
 * gh_zero_engine — Ministral generation tests
...
struct DecodeSession {
    samples: RefCell<VecDeque<u32>>,
    prefills: Cell<usize>,
    argmaxes: Cell<usize>,
    fused: RefCell<Vec<(u32, usize)>>,
...
#[cfg(feature = "cpu")]
impl DecodeSession {
    fn new(samples: impl IntoIterator<Item = u32>) -> Self {
        Self {
            samples: RefCell::new(samples.into_iter().collect()),
            prefills: Cell::new(0),
            argmaxes: Cell::new(0),
            fused: RefCell::new(Vec::new()),
    ...
    fn next(&self) -> u32 {
        self.samples.borrow_mut().pop_front().expect("test sample")
    }
}

#[cfg(feature = "cpu")]
impl crate::runtime::RuntimeSession for DecodeSession {
    type Graph = crate::family::mistral::graph::MistralGraph;
    ...
    fn token_argmax(&self, token: u32, position: usize, _: usize) -> Result<u32> {
        self.fused.borrow_mut().push((token, position));
        Ok(self.next())
    }

    fn logits(&self, vocab: usize) -> Result<Vec<f32>> {
        self.logits.set(self.logits.get() + 1);
        let mut logits = vec![f32::NEG_INFINITY; vocab];
        logits[self.next() as usize] = 0.0;
        Ok(logits)
    }

    fn argmax(&self, _: usize) -> Result<u32> {
        self.argmaxes.set(self.argmaxes.get() + 1);
        Ok(self.next())
    }

    fn topk(&self, _: usize, _: usize) -> Result<Vec<(u32, f32)>> {
        self.topks.set(self.topks.get() + 1);
        Ok(vec![(self.next(), 0.0)])
    }
}

#[cfg(feature = "cpu")]
fn runtime_model(context: usize) -> RuntimeModel {
    let tokenizer = crate::family::mistral::tokenizer::tests::mini_tokenizer();
    let (mut config, backend) = numeric::runtime_backend();
...
fn public_request(
    max_tokens: usize,
    sampling: crate::api::request::SamplingParams,
) -> crate::api::request::Request {
    crate::api::request::Request {
        messages: Vec::new(),
        sampling,
        max_tokens,
...
) -> (Option<GenerationStats>, Vec<Event>) {
    let mut events = Events::default();
    let result = {
        let mut terminal = Terminal::new(&mut events);
        let result = drive(model, request, prompt, session, &mut terminal).unwrap();
...
#[cfg(feature = "cpu")]
#[test]
fn greedy_fuses_only_steps_after_the_first_sample() {
    let model = runtime_model(8);
    let request = public_request(5, crate::api::request::SamplingParams::greedy());
    let session = DecodeSession::new([b'a' as u32, b'b' as u32, model.tokenizer.eos_id()]);
    let (stats, events) = run_drive(&model, &request, &[1, 2], &session);
    let stats = stats.expect("finished stats");

    assert_eq!(stats.prompt_tokens, 2);
    assert_eq!(stats.completion_tokens, 2);
    assert_eq!(session.prefills.get(), 1);
    assert_eq!(session.argmaxes.get(), 1);
    assert_eq!(
        session.fused.borrow().as_slice(),
        [(b'a' as u32, 2), (b'b' as u32, 3)]
    );
    assert!(session.tokens.borrow().is_empty());
    assert_eq!(
        events,
        [
...
#[cfg(feature = "cpu")]
#[test]
fn greedy_one_token_eos_and_context_edges_never_overrun() {
    let model = runtime_model(8);
    let one = DecodeSession::new([b'a' as u32]);
    let request = public_request(1, crate::api::request::SamplingParams::greedy());
    assert_eq!(
        run_drive(&model, &request, &[1, 2], &one)
            .0
            .unwrap()
            .completion_tokens,
        1
    );
    assert!(one.fused.borrow().is_empty());

    let eos = DecodeSession::new([model.tokenizer.eos_id()]);
    assert_eq!(
        run_drive(&model, &request, &[1, 2], &eos)
            .0
            .unwrap()
            .completion_tokens,
        0
    );
    assert!(eos.fused.borrow().is_empty());

    let short = runtime_model(3);
    let context = DecodeSession::new([b'a' as u32]);
    let request = public_request(4, crate::api::request::SamplingParams::greedy());
    assert_eq!(
        run_drive(&short, &request, &[1, 2], &context)
            .0
            .unwrap()
            .completion_tokens,
        1
    );
    assert!(context.fused.borrow().is_empty());
}

#[cfg(feature = "cpu")]
#[test]
fn topk_and_fallback_keep_plain_token_submission() {
    let model = runtime_model(8);
    let topk = DecodeSession::new([b'a' as u32, b'b' as u32]);
    let mut sampling = crate::api::request::SamplingParams::greedy();
    sampling.temperature = 1.0;
    sampling.top_k = 1;
    let request = public_request(2, sampling);
    assert_eq!(
        run_drive(&model, &request, &[1, 2], &topk)
            .0
            .unwrap()
            .completion_tokens,
        2
    );
    assert_eq!(topk.topks.get(), 2);
    assert_eq!(topk.tokens.borrow().as_slice(), [(b'a' as u32, 2)]);
    assert!(topk.fused.borrow().is_empty());

    let fallback = DecodeSession::new([b'a' as u32, b'b' as u32]);
    let mut sampling = crate::api::request::SamplingParams::greedy();
    sampling.repeat_penalty = 1.1;
    let request = public_request(2, sampling);
    assert_eq!(
        run_drive(&model, &request, &[1, 2], &fallback)
            .0
            .unwrap()
            .completion_tokens,
        2
    );
    assert_eq!(fallback.logits.get(), 2);
    assert_eq!(fallback.tokens.borrow().as_slice(), [(b'a' as u32, 2)]);
    assert!(fallback.fused.borrow().is_empty());
}
...
#[cfg(feature = "cpu")]
impl EventSink for CancelChecks {
    fn cancelled(&self) -> bool {
...
#[cfg(feature = "cpu")]
#[test]
fn greedy_cancellation_after_fused_completion_emits_no_next_token() {
    let model = runtime_model(8);
    let request = public_request(4, crate::api::request::SamplingParams::greedy());
    let session = DecodeSession::new([b'a' as u32, b'b' as u32]);
    let mut events = CancelChecks {
        cancel_at: 4,
        checks: Cell::new(0),
        values: Vec::new(),
    };
    let result = {
        let mut terminal = Terminal::new(&mut events);
        drive(&model, &request, &[1, 2], &session, &mut terminal).unwrap()
    };
    assert_eq!(result, None);
    assert_eq!(session.fused.borrow().as_slice(), [(b'a' as u32, 2)]);
    assert_eq!(events.values, [Event::TextDelta("a".into())]);
}
...
pub(crate) mod numeric {
    ...
    #[cfg(feature = "cpu")]
    pub(super) fn runtime_backend() -> (MistralConfig, CpuBackend) {
        let cfg = cfg();
        let backend = backend(&cfg, false, Profile::Dense);
        (cfg, backend)
    }
