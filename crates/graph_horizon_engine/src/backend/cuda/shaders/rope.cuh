...
extern "C" __global__ void cuda_rope(
    ...
    uint32_t position, uint32_t rows, float freq_base, float factor, float beta_fast,
    float beta_slow, float original_context, float scale) {
    const uint32_t half_rope = rope_dim / 2;
    const uint64_t pairs_per_row = uint64_t(heads) * half_rope;
    const uint64_t id = uint64_t(blockIdx.x) * blockDim.x + threadIdx.x;
    if (half_rope == 0 || id >= uint64_t(rows) * pairs_per_row) return;
    const uint32_t row = uint32_t(id / pairs_per_row);
    const uint32_t head = uint32_t(id % pairs_per_row) / half_rope;
    const uint32_t pair = uint32_t(id % half_rope);
    const uint64_t base = (uint64_t(row) * heads + head) * head_dim + pair * 2;
    ...
    const float extrapolated = float(position + row) * powf(freq_base, -2.0f * float(pair) / float(rope_dim));
