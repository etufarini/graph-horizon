...
kernel void metal_attention_prefill_matrix(device const half*q[[buffer(0)]],device const half*k[[buffer(1)]],device const half*v[[buffer(2)]],device half*out[[buffer(3)]],constant P&p[[buffer(4)]],uint id[[threadgroup_position_in_grid]],uint local[[thread_index_in_threadgroup]],uint lane[[thread_index_in_simdgroup]],uint group[[simdgroup_index_in_threadgroup]]){
 ...
 uint first=(id/p.qh)*8,h=id%p.qh,kh=h/4;for(uint index=local;index<8*128;index+=256){uint row=index/128,d=index%128;tq[index]=q[((first+row)*p.qh+h)*128+d];}float m=-INFINITY,l=0.0f;simdgroup_float8x8 acc[2];for(uint part=0;part<2;part++)acc[part]=make_filled_simdgroup_matrix<float,8>(0.0f);threadgroup_barrier(mem_flags::mem_threadgroup);
 // Eight SIMD groups cooperatively form a C64 score tile. Each owns one query
 // row for online softmax and 16 output dimensions for the PV accumulation.
 uint row=group;for(uint tile=0;tile<=p.base+first+7;tile+=64){
  uint block=group;simdgroup_float8x8 scores=make_filled_simdgroup_matrix<float,8>(0.0f);ulong kb=(ulong(p.layer)*p.context*p.kvh+ulong(tile+block*8)*p.kvh+kh)*128;for(uint d=0;d<128;d+=8){simdgroup_half8x8 qm,km;simdgroup_load(qm,tq+d,128,0,false);simdgroup_load(km,k+kb+d,p.kvh*128,0,true);simdgroup_multiply_accumulate(scores,qm,km,scores);}simdgroup_store(scores,score+block*8,64,0,false);
  threadgroup_barrier(mem_flags::mem_threadgroup);
  uint t0=tile+2*lane,t1=t0+1;float s0=t0<=p.base+first+row?score[row*64+2*lane]*p.scale:-INFINITY,s1=t1<=p.base+first+row?score[row*64+2*lane+1]*p.scale:-INFINITY,nm=max(m,simd_max(max(s0,s1))),alpha=exp(m-nm),w0=isfinite(s0)?exp(s0-nm):0.0f,w1=isfinite(s1)?exp(s1-nm):0.0f;l=l*alpha+simd_sum(w0+w1);m=nm;prob[row*64+2*lane]=half(w0);prob[row*64+2*lane+1]=half(w1);if(lane<8)scale[row*8+lane]=row==lane?alpha:0.0f;
  threadgroup_barrier(mem_flags::mem_threadgroup);simdgroup_float8x8 rescale;simdgroup_load(rescale,scale,8,0,false);
  for(uint part=0;part<2;part++){simdgroup_float8x8 next;simdgroup_multiply(next,rescale,acc[part]);for(uint block=0;block<8;block++){simdgroup_half8x8 weights,values;simdgroup_load(weights,prob+block*8,64,0,false);ulong vb=(ulong(p.layer)*p.context*p.kvh+ulong(tile+block*8)*p.kvh+kh)*128+group*16+part*8;simdgroup_load(values,v+vb,p.kvh*128,0,false);simdgroup_multiply_accumulate(next,weights,values,next);}acc[part]=next;}
  threadgroup_barrier(mem_flags::mem_threadgroup);
 }
 if(lane<8)scale[row*8+lane]=row==lane?1.0f/l:0.0f;threadgroup_barrier(mem_flags::mem_threadgroup);simdgroup_float8x8 norm;simdgroup_load(norm,scale,8,0,false);
 for(uint part=0;part<2;part++){simdgroup_float8x8 normalized;simdgroup_multiply(normalized,norm,acc[part]);simdgroup_store(normalized,result+group*16+part*8,128,0,false);}threadgroup_barrier(mem_flags::mem_threadgroup);
 for(uint index=local;index<8*128;index+=256){uint row=index/128,d=index%128;out[((first+row)*p.qh+h)*128+d]=half(result[index]);}
}
