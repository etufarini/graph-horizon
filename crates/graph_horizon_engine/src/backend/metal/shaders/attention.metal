...
kernel void metal_attention_prefill_matrix(device const half*q[[buffer(0)]],device const half*k[[buffer(1)]],device const half*v[[buffer(2)]],device half*out[[buffer(3)]],constant P&p[[buffer(4)]],uint id[[threadgroup_position_in_grid]],uint lane[[thread_index_in_simdgroup]],uint group[[simdgroup_index_in_threadgroup]]){
 threadgroup half prob[4*8*8];threadgroup float score[4*8*8],scale[4*8*8],pm[4*8],pl[4*8],partial[4*8*8];
 uint first=(id/p.qh)*8,h=id%p.qh,kh=h/4,qb=(first*p.qh+h)*128,gb=group*64;float m=-INFINITY,l=0.0f;simdgroup_float8x8 acc[16];for(uint part=0;part<16;part++)acc[part]=make_filled_simdgroup_matrix<float,8>(0.0f);
 // SIMD groups own disjoint history tiles; this exposes query-head workgroups
 // while preserving the matrix QK/PV path and exact FP32 online-softmax state.
 for(uint tile=group*8;tile<=p.base+first+7;tile+=32){
  simdgroup_float8x8 scores=make_filled_simdgroup_matrix<float,8>(0.0f);ulong kb=(ulong(p.layer)*p.context*p.kvh+ulong(tile)*p.kvh+kh)*128;
  for(uint d=0;d<128;d+=8){simdgroup_half8x8 qm,km;simdgroup_load(qm,q+qb+d,p.qh*128,0,false);simdgroup_load(km,k+kb+d,p.kvh*128,0,true);simdgroup_multiply_accumulate(scores,qm,km,scores);}simdgroup_store(scores,score+gb,8,0,false);
  simdgroup_barrier(mem_flags::mem_threadgroup);float weights[8],alpha=1.0f;if(lane<8){float tm=-INFINITY;for(uint col=0;col<8;col++)if(tile+col<=p.base+first+lane)tm=max(tm,score[gb+lane*8+col]*p.scale);float nm=max(m,tm);alpha=exp(m-nm);float add=0.0f;for(uint col=0;col<8;col++){float w=tile+col<=p.base+first+lane?exp(score[gb+lane*8+col]*p.scale-nm):0.0f;weights[col]=w;add+=w;}l=l*alpha+add;m=nm;}
  if(lane<8)for(uint col=0;col<8;col++){prob[gb+lane*8+col]=half(weights[col]);scale[gb+lane*8+col]=lane==col?alpha:0.0f;}simdgroup_barrier(mem_flags::mem_threadgroup);
  simdgroup_half8x8 weights_matrix;simdgroup_float8x8 scale_matrix;simdgroup_load(weights_matrix,prob+gb,8,0,false);simdgroup_load(scale_matrix,scale+gb,8,0,false);ulong vb=(ulong(p.layer)*p.context*p.kvh+ulong(tile)*p.kvh+kh)*128;
  for(uint part=0;part<16;part++){simdgroup_float8x8 scaled;simdgroup_half8x8 values;simdgroup_multiply(scaled,scale_matrix,acc[part]);simdgroup_load(values,v+vb+part*8,p.kvh*128,0,false);simdgroup_multiply_accumulate(acc[part],weights_matrix,values,scaled);}
 }
 if(lane<8){pm[group*8+lane]=m;pl[group*8+lane]=l;}threadgroup_barrier(mem_flags::mem_threadgroup);
 for(uint part=0;part<16;part++){
  simdgroup_store(acc[part],partial+gb,8,0,false);threadgroup_barrier(mem_flags::mem_threadgroup);
  if(group==0)for(uint index=lane;index<64;index+=32){uint row=index/8,col=index%8;float top=-INFINITY;for(uint g=0;g<4;g++)top=max(top,pm[g*8+row]);float den=0.0f,num=0.0f;for(uint g=0;g<4;g++){float s=exp(pm[g*8+row]-top);den+=pl[g*8+row]*s;num+=partial[g*64+index]*s;}out[((first+row)*p.qh+h)*128+part*8+col]=half(num/den);}
