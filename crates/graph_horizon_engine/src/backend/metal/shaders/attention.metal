...
kernel void metal_attention_prefill_matrix(device const half*q[[buffer(0)]],device const half*k[[buffer(1)]],device const half*v[[buffer(2)]],device half*out[[buffer(3)]],constant P&p[[buffer(4)]],uint kh[[threadgroup_position_in_grid]],uint local[[thread_index_in_threadgroup]],uint lane[[thread_index_in_simdgroup]],uint group[[simdgroup_index_in_threadgroup]]){
 threadgroup half tk[8*128],tv[8*128],prob[32*8*8];threadgroup float score[32*8*8],scale[32*8*8];uint slot=group>>3,row8=group&7,h=kh*4+slot,first=row8*8,qb=(first*p.qh+h)*128,gb=group*64;float m=-INFINITY,l=0.0f;simdgroup_float8x8 acc[16];for(uint part=0;part<16;part++)acc[part]=make_filled_simdgroup_matrix<float,8>(0.0f);
 for(uint tile=0;tile<=p.base+63;tile+=8){
  if(local<8*128){uint t=tile+local/128,d=local%128;ulong b=(ulong(p.layer)*p.context*p.kvh+ulong(t)*p.kvh+kh)*128+d;if(t<p.context){tk[local]=k[b];tv[local]=v[b];}else{tk[local]=half(0.0h);tv[local]=half(0.0h);}}
  threadgroup_barrier(mem_flags::mem_threadgroup);simdgroup_float8x8 scores=make_filled_simdgroup_matrix<float,8>(0.0f);
  for(uint d=0;d<128;d+=8){simdgroup_half8x8 qm,km;simdgroup_load(qm,q+qb+d,p.qh*128,0,false);simdgroup_load(km,tk+d,128,0,true);simdgroup_multiply_accumulate(scores,qm,km,scores);}simdgroup_store(scores,score+gb,8,0,false);
  ...
  threadgroup_barrier(mem_flags::mem_threadgroup);if(lane<8)for(uint col=0;col<8;col++){prob[gb+lane*8+col]=half(weights[col]);scale[gb+lane*8+col]=lane==col?alpha:0.0f;}threadgroup_barrier(mem_flags::mem_threadgroup);
  simdgroup_half8x8 pm;simdgroup_float8x8 sm;simdgroup_load(pm,prob+gb,8,0,false);simdgroup_load(sm,scale+gb,8,0,false);for(uint part=0;part<16;part++){simdgroup_float8x8 scaled;simdgroup_half8x8 vm;simdgroup_multiply(scaled,sm,acc[part]);simdgroup_load(vm,tv+part*8,128,0,false);simdgroup_multiply_accumulate(acc[part],pm,vm,scaled);}threadgroup_barrier(mem_flags::mem_threadgroup);
 }
 if(lane<8)for(uint col=0;col<8;col++)scale[gb+lane*8+col]=lane==col?1.0f/l:0.0f;threadgroup_barrier(mem_flags::mem_threadgroup);simdgroup_float8x8 norm;simdgroup_load(norm,scale+gb,8,0,false);for(uint part=0;part<16;part++){simdgroup_float8x8 normalized;simdgroup_multiply(normalized,norm,acc[part]);simdgroup_store(normalized,scale+gb,8,0,false);threadgroup_barrier(mem_flags::mem_threadgroup);if(lane<8)for(uint col=0;col<8;col++)out[((first+lane)*p.qh+h)*128+part*8+col]=half(scale[gb+lane*8+col]);threadgroup_barrier(mem_flags::mem_threadgroup);}
}
