...
kernel void metal_attention_gqa_split(device const half*q[[buffer(0)]],device const half*k[[buffer(1)]],device const half*v[[buffer(2)]],device float*partial[[buffer(3)]],device float*state[[buffer(4)]],constant P&p[[buffer(5)]],uint id[[threadgroup_position_in_grid]],uint lane[[thread_index_in_simdgroup]],uint group[[simdgroup_index_in_threadgroup]]){
 threadgroup half tk[32*128],tv[32*128];uint local=group*32+lane,kh=id,slot=group>>2,band=group&3,h=kh*4+slot,segment=band*4+(lane>>3),sub=lane&7,qb=h*128;float m=-INFINITY,l=0.0f,acc[16];
 for(uint part=0;part<16;part++)acc[part]=0.0f;
 for(uint tile=0;tile<=p.base;tile+=32){
  for(uint index=local;index<32*128;index+=512){uint t=tile+index/128,d=index%128;if(t<p.context){ulong b=(ulong(p.layer)*p.context*p.kvh+ulong(t)*p.kvh+kh)*128+d;tk[index]=k[b];tv[index]=v[b];}else{tk[index]=half(0.0h);tv[index]=half(0.0h);}}
  threadgroup_barrier(mem_flags::mem_threadgroup);
  for(uint offset=segment;offset<32&&tile+offset<=p.base;offset+=16){float dot=0.0f;for(uint part=0;part<16;part++){uint d=sub+part*8;dot+=float(q[qb+d])*float(tk[offset*128+d]);}dot+=simd_shuffle_xor(dot,4);dot+=simd_shuffle_xor(dot,2);dot+=simd_shuffle_xor(dot,1);float score=dot*p.scale,nm=max(m,score),a=exp(m-nm),weight=exp(score-nm);l=l*a+weight;for(uint part=0;part<16;part++){uint d=sub+part*8;acc[part]=acc[part]*a+weight*float(tv[offset*128+d]);}m=nm;}
  threadgroup_barrier(mem_flags::mem_threadgroup);
 }
 float m0=simd_shuffle(m,0),m1=simd_shuffle(m,8),m2=simd_shuffle(m,16),m3=simd_shuffle(m,24),gm=max(max(m0,m1),max(m2,m3));float r0=exp(m0-gm),r1=exp(m1-gm),r2=exp(m2-gm),r3=exp(m3-gm),gl=simd_shuffle(l,0)*r0+simd_shuffle(l,8)*r1+simd_shuffle(l,16)*r2+simd_shuffle(l,24)*r3,merged;uint part_id=h*4+band;
...
kernel void metal_attention_gqa_reduce(device const float*partial[[buffer(0)]],device const float*state[[buffer(1)]],device half*out[[buffer(2)]],constant P&p[[buffer(3)]],uint id[[threadgroup_position_in_grid]],uint lane[[thread_index_in_threadgroup]]){
 uint first=id*4;float top=max(max(state[first*2],state[(first+1)*2]),max(state[(first+2)*2],state[(first+3)*2]));float scales[4],den=0.0f;for(uint part=0;part<4;part++){scales[part]=exp(state[(first+part)*2]-top);den+=state[(first+part)*2+1]*scales[part];}for(uint part=0;part<4;part++){uint d=lane+part*32;float num=0.0f;for(uint split=0;split<4;split++)num+=partial[(first+split)*128+d]*scales[split];out[id*128+d]=half(num/den);}
}
