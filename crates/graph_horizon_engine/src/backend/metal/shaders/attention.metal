...
kernel void metal_attention(device const half*q[[buffer(0)]],device const uchar*k[[buffer(1)]],device const uchar*v[[buffer(2)]],device half*out[[buffer(3)]],constant P&p[[buffer(4)]],uint tid[[thread_position_in_grid]],uint id[[threadgroup_position_in_grid]],uint lane[[thread_index_in_simdgroup]],uint lanes[[threads_per_simdgroup]],uint group[[simdgroup_index_in_threadgroup]]){
 ...
 if(p.mode==3){
  // Four SIMD-groups retain the established accumulator width while sixteen
  // subgroups divide the time axis; threadgroup memory merges every state.
  threadgroup float pm[4],pl[4],pa[512];uint segment=group*4+(lane>>3),sub=lane&7;float m=-INFINITY,l=0.0f,acc[16];
  for(uint part=0;part<16;part++)acc[part]=0.0f;
  for(uint t=segment;t<=pos;t+=16){
  ...
  threadgroup_barrier(mem_flags::mem_threadgroup);
  if(group==0&&segment==0){float top=max(max(pm[0],pm[1]),max(pm[2],pm[3])),s0=exp(pm[0]-top),s1=exp(pm[1]-top),s2=exp(pm[2]-top),s3=exp(pm[3]-top),den=pl[0]*s0+pl[1]*s1+pl[2]*s2+pl[3]*s3;for(uint part=0;part<16;part++){uint d=sub+part*8;out[qb+d]=half((pa[d]*s0+pa[128+d]*s1+pa[256+d]*s2+pa[384+d]*s3)/den);}}return;
 }
 if(p.mode==2){
