...
kernel void metal_attention_prefill_matrix(device const half*q[[buffer(0)]],device const half*k[[buffer(1)]],device const half*v[[buffer(2)]],device half*out[[buffer(3)]],constant P&p[[buffer(4)]],uint id[[threadgroup_position_in_grid]],uint local[[thread_index_in_threadgroup]],uint lane[[thread_index_in_simdgroup]],uint group[[simdgroup_index_in_threadgroup]]){
 ...
 for(uint tile=0;tile<=p.base+first+7;tile+=64){
  for(uint block=group;block<8;block+=4){simdgroup_float8x8 scores=make_filled_simdgroup_matrix<float,8>(0.0f);ulong kb=(ulong(p.layer)*p.context*p.kvh+ulong(tile+block*8)*p.kvh+kh)*128;for(uint d=0;d<128;d+=8){simdgroup_half8x8 qm,km;simdgroup_load(qm,tq+d,128,0,false);simdgroup_load(km,k+kb+d,p.kvh*128,0,true);simdgroup_multiply_accumulate(scores,qm,km,scores);}simdgroup_store(scores,score+block*8,64,0,false);}
  ...
  for(uint part=0;part<4;part++){simdgroup_float8x8 next;simdgroup_multiply(next,rescale,acc[part]);for(uint block=0;block<8;block++){simdgroup_half8x8 weights,values;simdgroup_load(weights,prob+block*8,64,0,false);ulong vb=(ulong(p.layer)*p.context*p.kvh+ulong(tile+block*8)*p.kvh+kh)*128+group*32+part*8;simdgroup_load(values,v+vb,p.kvh*128,0,false);simdgroup_multiply_accumulate(next,weights,values,next);}acc[part]=next;}
  threadgroup_barrier(mem_flags::mem_threadgroup);
 }
...
kernel void metal_attention_gqa_matrix_split(device const half*q[[buffer(0)]],device const half*k[[buffer(1)]],device const half*v[[buffer(2)]],device float*partial[[buffer(3)]],device float*state[[buffer(4)]],constant P&p[[buffer(5)]],uint2 id[[threadgroup_position_in_grid]],uint local[[thread_index_in_threadgroup]],uint lane[[thread_index_in_simdgroup]],uint group[[simdgroup_index_in_threadgroup]]){
 threadgroup half tq[8*128],prob[8*64];threadgroup float score[8*64],scale[8*8],result[8*128];uint kh=id.x,split=id.y;
 for(uint index=local;index<8*128;index+=128){uint row=index/128,d=index%128;tq[index]=row<4?q[(kh*4+row)*128+d]:half(0.0h);}float m=-INFINITY,l=0.0f;simdgroup_float8x8 acc[4];for(uint part=0;part<4;part++)acc[part]=make_filled_simdgroup_matrix<float,8>(0.0f);threadgroup_barrier(mem_flags::mem_threadgroup);
 // Four SIMD groups form one exact four-query GQA tile. Padded rows remain
 // zero while four disjoint workgroups retain long-history parallelism.
 for(uint tile=split*64;tile<=p.base;tile+=4*64){
  for(uint block=group;block<8;block+=4){simdgroup_float8x8 scores=make_filled_simdgroup_matrix<float,8>(0.0f);ulong kb=(ulong(p.layer)*p.context*p.kvh+ulong(tile+block*8)*p.kvh+kh)*128;for(uint d=0;d<128;d+=8){simdgroup_half8x8 qm,km;simdgroup_load(qm,tq+d,128,0,false);simdgroup_load(km,k+kb+d,p.kvh*128,0,true);simdgroup_multiply_accumulate(scores,qm,km,scores);}simdgroup_store(scores,score+block*8,64,0,false);}
  threadgroup_barrier(mem_flags::mem_threadgroup);uint row=group,t0=tile+2*lane,t1=t0+1;float s0=t0<=p.base?score[row*64+2*lane]*p.scale:-INFINITY,s1=t1<=p.base?score[row*64+2*lane+1]*p.scale:-INFINITY,nm=max(m,simd_max(max(s0,s1))),alpha=exp(m-nm),w0=isfinite(s0)?exp(s0-nm):0.0f,w1=isfinite(s1)?exp(s1-nm):0.0f;l=l*alpha+simd_sum(w0+w1);m=nm;prob[row*64+2*lane]=half(w0);prob[row*64+2*lane+1]=half(w1);prob[(row+4)*64+2*lane]=half(0.0h);prob[(row+4)*64+2*lane+1]=half(0.0h);if(lane<8){scale[row*8+lane]=row==lane?alpha:0.0f;scale[(row+4)*8+lane]=0.0f;}
  threadgroup_barrier(mem_flags::mem_threadgroup);simdgroup_float8x8 rescale;simdgroup_load(rescale,scale,8,0,false);
  for(uint part=0;part<4;part++){simdgroup_float8x8 next;simdgroup_multiply(next,rescale,acc[part]);for(uint block=0;block<8;block++){simdgroup_half8x8 weights,values;simdgroup_load(weights,prob+block*8,64,0,false);ulong vb=(ulong(p.layer)*p.context*p.kvh+ulong(tile+block*8)*p.kvh+kh)*128+group*32+part*8;simdgroup_load(values,v+vb,p.kvh*128,0,false);simdgroup_multiply_accumulate(next,weights,values,next);}acc[part]=next;}
  threadgroup_barrier(mem_flags::mem_threadgroup);
 }
 uint row=group;if(lane<8){scale[row*8+lane]=row==lane?1.0f/l:0.0f;scale[(row+4)*8+lane]=0.0f;}threadgroup_barrier(mem_flags::mem_threadgroup);simdgroup_float8x8 norm;simdgroup_load(norm,scale,8,0,false);
 for(uint part=0;part<4;part++){simdgroup_float8x8 normalized;simdgroup_multiply(normalized,norm,acc[part]);simdgroup_store(normalized,result+group*32+part*8,128,0,false);}threadgroup_barrier(mem_flags::mem_threadgroup);
 if(lane==0){uint part_id=(kh*4+row)*8+split;state[part_id*2]=m;state[part_id*2+1]=l;}for(uint index=local;index<4*128;index+=128){uint output_row=index/128,d=index%128,part_id=(kh*4+output_row)*8+split;partial[part_id*128+d]=result[index];}
}
...
kernel void metal_attention_gqa_reduce(device const float*partial[[buffer(0)]],device const float*state[[buffer(1)]],device half*out[[buffer(2)]],constant P&p[[buffer(3)]],uint id[[threadgroup_position_in_grid]],uint lane[[thread_index_in_threadgroup]]){
 uint splits=p.mode==4?4:8,first=id*8;float top=-INFINITY;for(uint split=0;split<splits;split++)top=max(top,state[(first+split)*2]);float scales[8],den=0.0f;for(uint split=0;split<splits;split++){scales[split]=exp(state[(first+split)*2]-top);den+=state[(first+split)*2+1]*scales[split];}for(uint part=0;part<4;part++){uint d=lane+part*32;float num=0.0f;for(uint split=0;split<splits;split++)num+=partial[(first+split)*128+d]*scales[split];out[id*128+d]=half(num/den);}
}
