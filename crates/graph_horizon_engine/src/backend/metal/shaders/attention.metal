...
kernel void metal_attention_gqa_split(device const half*q[[buffer(0)]],device const half*k[[buffer(1)]],device const half*v[[buffer(2)]],device float*partial[[buffer(3)]],device float*state[[buffer(4)]],constant P&p[[buffer(5)]],uint2 id[[threadgroup_position_in_grid]],uint lane[[thread_index_in_simdgroup]],uint group[[simdgroup_index_in_threadgroup]]){
 ...
 for(uint tile=split*32;tile<=p.base;tile+=4*32){
  ...
  threadgroup_barrier(mem_flags::mem_threadgroup);
  float scores[4],weights[4],nm=m;for(uint item=0;item<4;item++){uint offset=segment+item*8;if(tile+offset<=p.base){float dot=0.0f;for(uint part=0;part<16;part++){uint d=sub+part*8;dot+=float(q[qb+d])*float(tk[offset*128+d]);}dot+=simd_shuffle_xor(dot,4);dot+=simd_shuffle_xor(dot,2);dot+=simd_shuffle_xor(dot,1);scores[item]=dot*p.scale;nm=max(nm,scores[item]);}else scores[item]=-INFINITY;}float a=exp(m-nm),sum=0.0f;for(uint item=0;item<4;item++){weights[item]=exp(scores[item]-nm);sum+=weights[item];}l=l*a+sum;for(uint part=0;part<16;part++){uint d=sub+part*8,value=segment;float next=0.0f;for(uint item=0;item<4;item++){next+=weights[item]*float(tv[value*128+d]);value+=8;}acc[part]=acc[part]*a+next;}m=nm;
  threadgroup_barrier(mem_flags::mem_threadgroup);
 }
