...
kernel void metal_attention(device const half*q[[buffer(0)]],device const uchar*k[[buffer(1)]],device const uchar*v[[buffer(2)]],device half*out[[buffer(3)]],constant P&p[[buffer(4)]],uint tid[[thread_position_in_grid]],uint id[[threadgroup_position_in_grid]],uint lane[[thread_index_in_simdgroup]],uint lanes[[threads_per_simdgroup]],uint group[[simdgroup_index_in_threadgroup]]){
 ...
 if(p.mode==4||p.mode==6){
  threadgroup half tk[32*128],tv[32*128];
  if(p.mode==6){
   // One threadgroup owns one KV head. Its four SIMD groups own the four
   // associated query heads and reuse each global K/V tile across them.
   uint local=group*32+lane,kh=id,h=kh*4+group,pos=p.base,qb=h*128;float m=-INFINITY,l=0.0f,acc[4];
   for(uint part=0;part<4;part++)acc[part]=0.0f;
   for(uint tile=0;tile<=pos;tile+=32){
    ...
    for(uint offset=0;offset<32&&tile+offset<=pos;offset++){float dot=0.0f;for(uint part=0;part<4;part++){uint d=lane+part*32;dot+=float(q[qb+d])*float(tk[offset*128+d]);}float score=simd_sum(dot)*p.scale,nm=max(m,score),a=exp(m-nm),weight=exp(score-nm);l=l*a+weight;for(uint part=0;part<4;part++){uint d=lane+part*32;acc[part]=acc[part]*a+weight*float(tv[offset*128+d]);}m=nm;}
    threadgroup_barrier(mem_flags::mem_threadgroup);
   }
   for(uint part=0;part<4;part++){uint d=lane+part*32;out[qb+d]=half(acc[part]/l);}return;
  }
  ...
  uint local=group*32+lane,row0=(id/p.kvh)*4,kh=id%p.kvh,row=row0+group,h=kh*4+(lane>>3),sub=lane&7,pos=p.base+row,qb=(row*p.qh+h)*128;float m=-INFINITY,l=0.0f,acc[16];
