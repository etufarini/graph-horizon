...
struct P{ulong kp;ulong vp;ulong km;ulong vm;uint vectors;uint dim;uint scheme;uint context;uint kvh;};
...
kernel void metal_kv_write(device const half*k[[buffer(0)]],device const half*v[[buffer(1)]],device uchar*kc[[buffer(2)]],device uchar*vc[[buffer(3)]],constant P&p[[buffer(4)]],uint vec[[thread_position_in_grid]]){if(vec>=p.vectors)return;if(p.scheme==0){ulong first=p.kp/(2*p.dim),layer=first/(p.context*p.kvh),pos=(first/p.kvh)%p.context,token=vec/p.kvh,kh=vec%p.kvh,dst=((layer*p.kvh+kh)*p.context+pos+token)*p.dim;for(uint i=0;i<p.dim;i++){((device half*)kc)[dst+i]=k[vec*p.dim+i];((device half*)vc)[dst+i]=v[vec*p.dim+i];}}else{quant(k,kc,p.kp,p.km,vec,p.dim);quant(v,vc,p.vp,p.vm,vec,p.dim);}}
