...
kernel void metal_matmul_q4(device const half*a[[buffer(0)]],device const uchar*w[[buffer(1)]],device uchar*out[[buffer(2)]],constant Params&p[[buffer(3)]],uint tid[[thread_position_in_grid]],ushort sl[[thread_index_in_simdgroup]]){
 ...
 for(uint row=0;row<2&&first+row<p.output;row++){float total=simd_sum(sum[row]);if(sl==0){if(p.fp32)((device float*)out)[first+row]=total;else((device half*)out)[first+row]=half(total);}}
}
kernel void metal_matmul_q6(device const half*a[[buffer(0)]],device const uchar*w[[buffer(1)]],device uchar*out[[buffer(2)]],constant Params&p[[buffer(3)]],uint tid[[thread_position_in_grid]],ushort sl[[thread_index_in_simdgroup]]){
