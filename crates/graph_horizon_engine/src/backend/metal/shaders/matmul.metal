...
kernel void metal_matmul(device const half*a[[buffer(0)]],device const uchar*w[[buffer(1)]],device uchar*out[[buffer(2)]],constant Params&p[[buffer(3)]],uint tid[[thread_position_in_grid]]){
 bool parallel=p.format==1||p.format==3;uint lane=parallel?(tid&3):0,o=parallel?(tid>>2):tid;
 if(!parallel&&o>=p.output)return;float acc=0.0f;uint ns=p.input/256;
 if(p.format==1){
  if(o<p.output)for(uint s=0;s<ns;s++){
   uint b=(o*ns+s)*144;float d=h(w,b),dm=h(w,b+2);
   for(uint j=lane;j<8;j+=4){
 ...
 if(parallel){
  // Four adjacent lanes own disjoint input groups for one output row.
  acc+=simd_shuffle_down(acc,2);acc+=simd_shuffle_down(acc,1);
  if(lane!=0||o>=p.output)return;
 }
