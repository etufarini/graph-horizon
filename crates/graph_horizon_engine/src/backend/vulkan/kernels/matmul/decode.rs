...
mod tests {
    ...
    #[test]
    fn mmq_batch_q4k_matches_cpu_oracle_with_tail_tiles() {
        let backend = match VulkanBackend::bare() {
            Ok(backend) => backend,
            Err(_) => {
                eprintln!("MMQ batch parity skipped: no Vulkan device");
                return;
            }
        };
        if !backend.dev.dp4a {
            eprintln!("MMQ batch parity skipped: device has no integer dot product");
            return;
        }
        let (in_dim, out_dim, rows) = (512usize, 70usize, 5usize);
        let mut seed = 0x62_61_74_63u32;
        let mut rng = || {
            seed = seed.wrapping_mul(1664525).wrapping_add(1013904223);
            seed
        };
        let mut weights = vec![0u8; out_dim * (in_dim / 256) * 144];
        ...
        let activations: Vec<f32> = (0..rows * in_dim)
            .map(|_| ((rng() >> 8) as f32 / u32::MAX as f32) - 0.5)
            .collect();
        let oracle_weights = CpuBuffer::from_bytes(weights.clone(), CpuFormat::Q4_K);
        let expected: Vec<f32> = activations
            .chunks_exact(in_dim)
            .flat_map(|row| compute::matmul(&oracle_weights, row, in_dim, out_dim))
            .collect();
        let activation_bytes = f32_to_f16_bytes(
            &activations
                .iter()
        ...
        backend
            ...
            .expect("upload input");
        let mut weight =
            GpuBuffer::alloc(&backend.dev, weights.len() as u64, false).expect("weight");
        weight.quant = WeightFormat::Q4K;
        backend
            .upload_bytes(&weight, &weights)
        ...
        for (index, bytes) in raw.chunks_exact(2).enumerate() {
            ...
            assert!(got.is_finite(), "element {index}: non-finite result");
            assert!(
                (got - want).abs() <= 1.5e-1 || (got - want).abs() <= 3e-2 * want.abs().max(1.0),
                "element {index}: got={got} want={want}"
            );
        }
        input.destroy(&backend.dev);
        weight.destroy(&backend.dev);
        output.destroy(&backend.dev);
    }
}
