...
`graph_orizon_engine` è il runtime di inferenza text-to-text per
...
```sh
cargo check -p graph_orizon_engine --no-default-features --features cpu
cargo check -p graph_orizon_engine --no-default-features --features vulkan
cargo check -p graph_orizon_engine --no-default-features --features vulkan-hybrid
cargo check -p graph_orizon_engine --no-default-features --features metal
cargo check -p graph_orizon_engine --no-default-features --features metal-hybrid
```
...
```rust
use graph_orizon_engine::{Engine, EngineConfig, Message, Request, Role, SamplingParams};
...
|---|---|
| `GRAPH_ORIZON_MODEL` | singolo GGUF reale |
| `GRAPH_ORIZON_MODEL_Q4_K_M` | artefatto Q4_K_M per la prova tokenizer |
| `GRAPH_ORIZON_REFERENCE_CLI` | eseguibile oracle per token greedy |
| `GRAPH_ORIZON_REFERENCE_TOKENIZE` | eseguibile oracle per tokenizzazione |
| `GRAPH_ORIZON_REFERENCE_PROMPT_IDS` | vettore prompt Reasoning fornito dallo script |
| `GRAPH_ORIZON_REFERENCE_COMPLETION_IDS` | 16 ID oracle forniti dallo script |

`GRAPH_ORIZON_CONTEXT`, `GRAPH_ORIZON_KV` e, nella prova mixed,
`GRAPH_ORIZON_VRAM_WEIGHTS_PERCENT` configurano la riga esterna senza individuare
risorse. Le variabili `GRAPH_ORIZON_REFERENCE_*_IDS` sono interne allo script di
parità e non sono configurazione runtime stabile.

```sh
GRAPH_ORIZON_MODEL="/path/model.gguf" GRAPH_ORIZON_CONTEXT=4096 GRAPH_ORIZON_KV=f16 \
GRAPH_ORIZON_REFERENCE_PROMPT_IDS="..." GRAPH_ORIZON_REFERENCE_COMPLETION_IDS="..." \
  cargo test --release -p graph_orizon_engine --no-default-features --features cpu \
