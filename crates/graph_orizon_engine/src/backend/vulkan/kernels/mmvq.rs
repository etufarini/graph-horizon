/*
 * graph_orizon_engine — host-side dispatch of the mmvq Q4_K decode GEMV
...
// `GRAPH_ORIZON_DECODE_MMVQ=1` (read once) while it is measured against the float GEMV: the
...
pub(crate) fn decode_mmvq_enabled() -> bool {
    ...
    *FLAG.get_or_init(|| {
        matches!(
            std::env::var("GRAPH_ORIZON_DECODE_MMVQ").ok().as_deref(),
