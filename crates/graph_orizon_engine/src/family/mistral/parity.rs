/*
 * graph_orizon_engine — selected-runtime parity protocol
...
pub(crate) fn validate(
    ...
    let expected_prompt = parse("GRAPH_ORIZON_REFERENCE_PROMPT_IDS", prompt_ids, None)?;
    let completion = parse(
        "GRAPH_ORIZON_REFERENCE_COMPLETION_IDS",
        completion_ids,
        Some(TOKEN_COUNT),
    )?;
