/*
 * graph_orizon_engine — Tekken tokenizer metadata contract
...
pub(crate) mod tests {
    ...
    fn pinned_differential_vectors_match_q4_artifact() {
        ...
        let binary = std::env::var("GRAPH_ORIZON_REFERENCE_TOKENIZE")
            .expect("GRAPH_ORIZON_REFERENCE_TOKENIZE required");
        let model =
            std::env::var("GRAPH_ORIZON_MODEL_Q4_K_M").expect("GRAPH_ORIZON_MODEL_Q4_K_M required");
        let corpus = [
