/*
 * graph_orizon_engine — final source and family boundary audit
...
use graph_orizon_engine::BackendMemory;
#[cfg(feature = "vulkan-hybrid")]
use graph_orizon_engine::PlacementReport;
...
fn removed_surface_scan() {
    ...
    for relative in [
        "crates/graph_orizon_engine/src/backend/cpu/kernels/matmul/q8_0.rs",
        "crates/graph_orizon_engine/src/backend/cpu/kernels/matmul/q8_0_simd.rs",
        "crates/graph_orizon_engine/src/backend/vulkan/shaders/embed/embed_q8_0.comp",
        "crates/graph_orizon_engine/src/backend/vulkan/shaders/logits/logits_q8_0.comp",
        "crates/graph_orizon_engine/src/backend/vulkan/shaders/matmul/matmul_q8_0.comp",
    ] {
        assert!(
    ...
    for relative in [
        ...
        "crates/graph_orizon_engine/examples/prefill_trace.rs",
        "crates/graph_orizon_engine/examples/quality.rs",
        "crates/graph_orizon_engine/examples/regression.rs",
        "crates/graph_orizon_engine/examples/validate.rs",
    ] {
        assert!(
            !repository().join(relative).exists(),
            "{relative} must be absent"
...
fn docs_contract() {
    ...
    let engine = fs::read_to_string(root.join("crates/graph_orizon_engine/README.md"))
    ...
    let ownership = fs::read_to_string(root.join("crates/graph_orizon_engine/src/README.md"))
...
fn real_selected_runtime_parity_and_lifecycle() {
    let model = std::env::var("GRAPH_ORIZON_MODEL").expect("GRAPH_ORIZON_MODEL required");
    let context = required_usize("GRAPH_ORIZON_CONTEXT");
    assert_eq!(context, 4096, "GRAPH_ORIZON_CONTEXT must be 4096");
    let kv = std::env::var("GRAPH_ORIZON_KV").expect("GRAPH_ORIZON_KV required");
    let kv_quant = KvQuant::parse(&kv).expect("GRAPH_ORIZON_KV must be f16 or int8");
    let percentage = std::env::var("GRAPH_ORIZON_VRAM_WEIGHTS_PERCENT")
        .ok()
    ...
    let prompt = std::env::var("GRAPH_ORIZON_REFERENCE_PROMPT_IDS")
        .expect("GRAPH_ORIZON_REFERENCE_PROMPT_IDS required");
    let completion = std::env::var("GRAPH_ORIZON_REFERENCE_COMPLETION_IDS")
        .expect("GRAPH_ORIZON_REFERENCE_COMPLETION_IDS required");
    let report = graph_orizon_engine::harness::validate_parity(&engine, &prompt, &completion)
...
fn assert_placement(engine: &Engine, percentage: Option<u8>) {
    #[cfg(any(feature = "vulkan-hybrid", feature = "metal-hybrid"))]
    {
        let expected = std::env::var("GRAPH_ORIZON_EXPECTED_MODE")
            .expect("GRAPH_ORIZON_EXPECTED_MODE required for hybrid profiles");
...
fn parity_request() -> Request {
    Request {
        ...
        sampling: graph_orizon_engine::SamplingParams::greedy(),
        max_tokens: 16,
    }
}
...
fn only_mistral3_reaches_the_family_contract() {
    ...
    let path = std::env::temp_dir().join(format!("graph_orizon_{suffix}.gguf"));
