/*
 * graph_orizon_engine — test-only Reasoning qualification harness
...
fn approved_reasoning_id(model_id: &str) -> Result<(), &'static str> {
    match model_id {
        ...
        _ => Err("unknown or non-Reasoning GRAPH_ORIZON_MODEL_ID"),
    }
}
...
fn real_semantic_acceptance() {
    let model_id = std::env::var("GRAPH_ORIZON_MODEL_ID").expect("GRAPH_ORIZON_MODEL_ID required");
