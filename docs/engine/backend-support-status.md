...
Metal qualification applies only to the stated M4/macOS tuple, not every Apple
GPU, Apple silicon generation, or operating-system version. The reviewed
matrix is limited to hardware available to the maintainer. An unavailable row
is external verification, not an implicit support or failure claim.

Supported artifacts are defined in
[supported models and formats](../supported-models-and-formats.md). Current
reviewed results are in
