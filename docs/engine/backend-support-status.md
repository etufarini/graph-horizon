...
There are two numeric backends: CPU and Vulkan. `vulkan-hybrid` composes
these backends; it is not an additional numeric backend.
...
Runtime admission and qualification are separate. A Vulkan device may
