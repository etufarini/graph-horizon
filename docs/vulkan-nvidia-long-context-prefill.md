...
## Fresh Prefill And Competitive Matrix

Graph Horizon reports public TTFT, which includes tokenization and first
sampling/emission. llama.cpp reports pure prompt processing. The difference is
not subtracted. All Graph Horizon CVs are below 0.75%.

| Context | TTFT mean | Median | Stddev | CV | Prompt tok/s | GPU prefill | llama.cpp | Ratio | Gap |
|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 128 | 205.06 ms | 205.77 ms | 1.34 ms | 0.66% | 624.23 | 166.55 ms | 97.71 ms | 2.10x | 107.35 ms |
| 512 | 582.77 ms | 581.70 ms | 2.25 ms | 0.39% | 878.57 | 535.74 ms | 285.80 ms | 2.04x | 296.97 ms |
| 2,048 | 2,238.81 ms | 2,239.32 ms | 2.87 ms | 0.13% | 914.77 | 2,195.13 ms | 1,175.59 ms | 1.90x | 1,063.22 ms |
| 8,192 | 10,033.02 ms | 10,025.37 ms | 24.99 ms | 0.25% | 816.51 | 10,112.00 ms | 5,279.64 ms | 1.90x | 4,753.38 ms |
| 15,435 | 22,061.52 ms | 22,066.17 ms | 15.29 ms | 0.07% | 699.63 | 22,050.36 ms | 11,419.29 ms | 1.93x | 10,642.23 ms |
| 28,000 | 48,436.05 ms | 48,464.98 ms | 54.62 ms | 0.11% | 578.08 | 48,622.31 ms | 24,727.57 ms | 1.96x | 23,708.48 ms |
...
transactions. The nominal 360 GB/s movement floor is far below measured time,
while prior geometry and cache probes show unpack, Matrix2 feed, shared state,
issue, and occupancy are material; raw weight-bandwidth tuning alone is closed.

Exact Q64 attention performs the same dense causal pairs with no score
materialization. At 28K its logical K/V traffic and useful QK/AV work are about
218 TB / 218 TFLOP across 34 layers. The retained Q64 architecture already
loads Q once, keeps FP32 online state live, removes score/probability shared
round trips, and uses exact Matrix2 tiles. Its acquired practical floor is
15.990 seconds on this exact device/driver; the fresh current time is 19.691
seconds.

| Rank | Target | Current | Physical / observed lower bound | Practical floor | Realistically removable | Amdahl result / decision |
|---:|---|---:|---:|---:|---:|---|
| 1 | Exact attention | 19.691 s | 9.186 s llama observed; 2.74 s dense-MMA-only bound excludes loads/state | 15.990 s | <=3.701 s | `f=0.4065`, `S_local=1.231`, final 44.735 s / 625.9 tok/s, 1.083x whole; below 10% complex gate |
...
| Prompt | Baseline TTFT | Final TTFT | Delta | Final prompt tok/s |
|---:|---:|---:|---:|---:|
| 128 | 205.06 ms | 205.06 ms | 0% | 624.23 |
| 512 | 582.77 ms | 582.77 ms | 0% | 878.57 |
| 2K | 2,238.81 ms | 2,238.81 ms | 0% | 914.77 |
| 8K | 10,033.02 ms | 10,033.02 ms | 0% | 816.51 |
| 15K | 22,061.52 ms | 22,061.52 ms | 0% | 699.63 |
| 28K | 48,436.05 ms | 48,436.05 ms | 0% | 578.08 |

| KV / prompt | Baseline model tok/s | Final | Delta |
|---:|---:|---:|---:|
| 128 | 35.94 | identical source | 0% |
| 2K | 34.53 | identical source | 0% |
| 8K | 31.31 | identical source | 0% |
| 15K | 28.41 | identical source | 0% |
| 28K | 24.47 | identical source | 0% |
...
| Completion tokens / public deltas | 954 / 954 |
| TTFT / prefill boundary | 21.702 s |
| Prefill throughput | 711.23 tok/s |
| Decode throughput | 27.62 model tok/s |
| Decode time | about 34.54 s |
| Total request | about 56.24 s |
