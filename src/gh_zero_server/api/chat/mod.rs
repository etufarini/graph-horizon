/*
 * GH Zero headless server - API - Chat (OpenAI chat wire format)
 * Single responsibility: map validated text-only chat JSON to engine requests
 * and engine text events to SSE lines. It depends on gh_zero_engine API types
 * and does not include tools, tool_choice, workspace, or reasoning controls.
*/

use gh_zero_engine::{Event, Message, Request, SamplingParams};
use serde::Deserialize;

use super::{ApiError, role_from_str};
use crate::gh_zero_server::ServerConfig;
...
mod tests {
    ...
    fn empty_deltas_and_finished_are_skipped() {
        ...
        assert!(
            event_line(&Event::Finished(gh_zero_engine::GenerationStats {
                prompt_tokens: 128,
