 ...
 * It never changes category, fetches result URLs, or falls back to no search.
 */
...
impl State {
    pub(in crate::graph_horizon_web) fn new(config: Config) -> State {
        State {
    ...
    pub(in crate::graph_horizon_web) fn capability(&self) -> Capability {
        Capability {
            provider: self.config.capability_label().to_string(),
    ...
    pub(super) async fn context(&self, request: &Request) -> Result<context::Framed, Error> {
        ...
        let results =
            provider::search(&self.config, request)
                .await
        ...
        context::frame(
            &results,
            request,
            provider::label(&self.config, request.category()),
        )
