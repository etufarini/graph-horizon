/*
 * Graph Orizon web router
 * Single responsibility: route static assets and delegate chat requests to the
 * headless server pipeline. It depends on assets and graph_orizon_server state, and
...
use crate::graph_orizon_server::{ResponseBody, ServerState};
...
pub(super) async fn handle(
    ...
    let response = match (req.method(), req.uri().path()) {
        ...
        (&Method::POST, "/v1/chat/completions") => {
            crate::graph_orizon_server::handler::chat_completions(req, chat).await
        }
