...
usage() {
    printf '%s\n' \
        "usage: install.sh --backend cpu|vulkan|vulkan-hybrid [--profile release|fast] [--prefix PATH]"
}
...
[[ -n "${backend}" ]] \
    || fail "--backend is required; accepted: cpu, vulkan, vulkan-hybrid"
case "${backend}" in
    cpu|vulkan|vulkan-hybrid) ;;
    *) fail "invalid backend: ${backend}; accepted: cpu, vulkan, vulkan-hybrid" ;;
esac
case "${profile}" in
    release|fast) ;;
...
case "${os}/${arch}/${backend}" in
    Darwin/arm64/cpu|Darwin/arm64/vulkan|Darwin/arm64/vulkan-hybrid) ;;
    Linux/x86_64/cpu|Linux/x86_64/vulkan|Linux/x86_64/vulkan-hybrid) ;;
    *) fail "unsupported platform/backend: ${os}/${arch}/${backend}" ;;
esac
