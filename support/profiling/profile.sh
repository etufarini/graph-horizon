...
while (($#)); do
    case "$1" in
        ...
        --help|-h)
            echo "usage: profile.sh --model PATH --backend cpu|vulkan|vulkan-hybrid|metal|metal-hybrid|cuda --context N --kv f16|int8 [--weights-percent 0..100]"
            exit 0
            ;;
...
case "$backend" in cpu|vulkan|vulkan-hybrid|metal|metal-hybrid|cuda) ;; *) fail "invalid backend" ;; esac
case "$kv" in f16|int8) ;; *) fail "invalid KV scheme" ;; esac
[[ "$context" =~ ^[1-9][0-9]*$ ]] || fail "context must be >= 1"
if [[ -n "$weights_percent" ]]; then
    [[ "$backend" == vulkan-hybrid || "$backend" == metal-hybrid ]] \
