...
while (($#)); do
    case "$1" in
        ...
        --help|-h)
            echo "usage: profile.sh --model PATH --backend cpu|vulkan|vulkan-hybrid|metal|metal-hybrid|cuda|cuda-hybrid --context N --kv f16|int8 [--weights-percent 0..100]"
            exit 0
            ;;
...
if [[ -n "$weights_percent" ]]; then
    [[ "$backend" == vulkan-hybrid || "$backend" == metal-hybrid || "$backend" == cuda-hybrid ]] \
