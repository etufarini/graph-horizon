...
test_status=$?
set -e
if grep -Eq '(external verification: )?(Vulkan|Metal|CUDA) backend (is )?unavailable' "$temporary_dir/test.log"; then external "$backend backend unavailable"; fi
if ((test_status != 0)); then
    if grep -Eq '(Vulkan|Metal|CUDA) memory is insufficient|context 4096 does not fit|model does not fit available RAM and VRAM|mixed placement required' "$temporary_dir/test.log"; then
