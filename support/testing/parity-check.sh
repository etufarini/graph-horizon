...
usage() {
    echo "usage: parity-check.sh --models-dir DIR --model-id ID --backend cpu|vulkan|vulkan-hybrid --kv f16|int8 --reference-server PATH [--reference-port PORT] [--weights-percent 0..100 --expect-mode all-gpu|mixed|cpu-only]"
}
...
case "$backend" in cpu|vulkan|vulkan-hybrid) ;; *) usage_error "invalid backend" ;; esac
...
test_status=$?
set -e
if grep -Eq '(external verification: )?Vulkan backend (is )?unavailable' "$temporary_dir/test.log"; then external "$backend backend unavailable"; fi
