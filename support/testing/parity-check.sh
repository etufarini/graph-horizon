...
reference_port="18080"; server_pid=""; temporary_dir=""
...
usage() {
    echo "usage: parity-check.sh --models-dir DIR --model-id ID --backend cpu|vulkan|hybrid --kv f16|int8 --reference-server PATH [--reference-port PORT]"
}
...
[[ -n "$models_dir" && -n "$model_id" && -n "$reference_server" ]] || usage_error "required argument is missing"
...
END {
    if (!bad && rows != 6) exit 2
    if (!bad && selected == "") exit 3
    if (!bad) print selected
}' "$catalog")"
catalog_status=$?
set -e
((catalog_status == 0)) || {
...
size="$(stat -c %s -- "$model" 2>/dev/null)" || external "$model_id byte count unavailable"
[[ "$size" == "$expected_bytes" ]] || external "$model_id byte count mismatch"
digest="$(sha256sum -- "$model" 2>/dev/null)" || external "$model_id SHA-256 unavailable"
[[ "${digest%% *}" == "$expected_hash" ]] || external "$model_id SHA-256 mismatch"
...
post_json() {
    ...
    curl --fail --silent --show-error -H 'Content-Type: application/json' \
        --data-binary "$body" "http://127.0.0.1:$reference_port/$endpoint" >"$output" \
        || real_failure "oracle HTTP request failed"
}
...
body="$(jq -cn --arg content "$oracle_prompt" \
    '{content:$content,add_special:true,parse_special:true}')"
post_json tokenize "$body" "$temporary_dir/tokenize.json"
prompt_ids="$(jq -er '.tokens | select(type == "array" and length > 0) |
    select(all(.[]; type == "number" and . >= 0 and floor == .)) | map(tostring) | join(",")' \
    "$temporary_dir/tokenize.json")" || real_failure "malformed tokenize response"
body="$(jq -cn --argjson prompt "[$prompt_ids]" \
    '{prompt:$prompt,n_predict:16,temperature:0,top_k:1,top_p:1,min_p:0,repeat_penalty:1,stream:false,return_tokens:true}')"
post_json completion "$body" "$temporary_dir/completion.json"
completion_ids="$(jq -er '.tokens | select(type == "array" and length == 16) |
    select(all(.[]; type == "number" and . >= 0 and floor == .)) | map(tostring) | join(",")' \
    "$temporary_dir/completion.json")" || real_failure "malformed completion response"

case "$backend" in
    cpu) test_name='family::mistral::run::tests::real_ministral_parity' ;;
    vulkan) test_name='family::mistral::vulkan::real_ministral_parity' ;;
    hybrid) test_name='family::mistral::hybrid::graph::real_ministral_parity' ;;
esac
set +e
(
    cd "$project_dir"
    GH_ZERO_MODEL="$model" GH_ZERO_CONTEXT=4096 GH_ZERO_KV="$kv" \
    GH_ZERO_VRAM_WEIGHTS_PERCENT=25 GH_ZERO_REFERENCE_PROMPT_IDS="$prompt_ids" \
    GH_ZERO_REFERENCE_COMPLETION_IDS="$completion_ids" \
        cargo test --locked -p gh_zero_engine --no-default-features --features "$backend" \
            "$test_name" -- --ignored --nocapture --exact
) >"$temporary_dir/test.log" 2>&1
test_status=$?
set -e
if ((test_status != 0)); then
    if grep -Eq 'Vulkan memory is insufficient|context 4096 does not fit|model does not fit available RAM and VRAM|mixed placement required' "$temporary_dir/test.log"; then
        external "insufficient memory for $backend row"
    fi
    real_failure "local parity assertion failed"
fi
grep -Fq 'external verification: Vulkan backend unavailable' "$temporary_dir/test.log" \
