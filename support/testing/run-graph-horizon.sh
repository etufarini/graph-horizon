...
while (($#)); do
    case "$1" in
        ...
        --help|-h)
            echo "usage: run-graph-horizon.sh --model PATH --backend cpu|vulkan|vulkan-hybrid --context N --kv f16|int8 [--max-tokens N]"
            exit 0
            ;;
...
case "$backend" in cpu|vulkan|vulkan-hybrid) ;; *) fail "invalid backend" ;; esac
