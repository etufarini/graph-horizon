...
while (($#)); do
    case "$1" in
        ...
        --help|-h)
            echo "usage: run-graph-orizon.sh --model PATH --backend cpu|vulkan|vulkan-hybrid|metal|metal-hybrid --context N --kv f16|int8 [--max-tokens N]"
            exit 0
            ;;
...
exec cargo run --locked --no-default-features --features "$backend" \
    --bin graph-orizon -- --provider local --model "$model" \
