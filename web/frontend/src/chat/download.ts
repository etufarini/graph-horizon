 ...
 * owns the exported chat file naming (gh-zero-chat-<timestamp>.json),
...
export function downloadChatFile(jsonText: string): void {
  ...
  // Local time, zero-padded: gh-zero-chat-YYYYMMDD-HHMMSS.json. Same-second
  ...
  anchor.href = url;
  anchor.download = `gh-zero-chat-${timestamp}.json`;
