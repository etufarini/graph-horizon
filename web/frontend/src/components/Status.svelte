...
<style lang="scss">
  ...
  .status-message { border: var(--gn-rule-width) solid currentColor; border-radius: var(--gn-radius-sm); padding: var(--gn-space-sm); font-size: var(--gn-text-sm); font-weight: 600; }
  ...
  .status-panel { min-height: var(--gn-control-height); box-sizing: border-box; display: flex; align-items: center; gap: var(--gn-space-sm); border: var(--gn-rule-width) solid var(--gn-border-subtle); border-radius: var(--gn-radius-sm); background: var(--gn-bg-panel); padding: var(--gn-space-xs) var(--gn-space-sm); color: var(--gn-text-muted); font: 650 var(--gn-text-xs) var(--gn-font-mono); font-variant-numeric: tabular-nums; }
  .status-panel span { min-width: 0; flex: 1 1 auto; overflow-wrap: anywhere; }
  .context-track { width: clamp(72px, 24%, 180px); height: 6px; flex: 0 0 auto; border: var(--gn-rule-width) solid var(--gn-border); border-radius: 999px; background: var(--gn-bg-panel-raised); overflow: hidden; }
