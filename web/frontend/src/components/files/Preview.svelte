...
<section class="preview" aria-label={`Preview ${file.name}`}>
  <header>
    ...
    <details>
      <summary aria-label={`Actions for ${file.name}`}>Actions</summary>
      <div class="actions">
...
<style lang="scss">
  ...
  header { min-width: 0; display: flex; align-items: center; gap: var(--gn-space-sm); padding: var(--gn-space-xs) var(--gn-space-2xs); }
  ...
  details { position: relative; flex: 0 0 auto; }
  summary, button { min-height: var(--gn-control-height); border: var(--gn-rule-width) solid var(--gn-border); background: var(--gn-bg-panel); padding: var(--gn-space-xs) var(--gn-space-sm); color: var(--gn-text-muted); cursor: pointer; font: 650 var(--gn-text-xs) var(--gn-font-sans); }
  summary { display: flex; align-items: center; list-style: none; }
  summary::-webkit-details-marker { display: none; }
  summary::after { content: " ▾"; }
  details[open] summary::after { content: " ▴"; }
  .actions { position: absolute; z-index: 3; top: calc(100% + var(--gn-space-xs)); right: 0; display: flex; gap: var(--gn-space-xs); border: var(--gn-border-width) solid var(--gn-border); background: var(--gn-bg-panel); box-shadow: var(--gn-shadow-hard); padding: var(--gn-space-xs); }
  ...
  @media (max-width: 640px) { summary, button { min-height: var(--gn-touch-height); } }
</style>
